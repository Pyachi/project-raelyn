...
const Enemy Enemy::ENEMYTEST1(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
  if (enemy->cycle(200, 50))
    enemy->moveTo({150, -300}, 50, QUICK);
  else if (enemy->cycle(200, 150))
    enemy->moveTo({-150, -300}, 50);
});
const Enemy Enemy::ENEMYTEST2(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
  if (enemy->getAge() < 60)
...
const Enemy
		Enemy::TANKYBANG_UP_DOWN(Texture::ENEMYTEMP, 600, [](EntityEnemy* enemy) {
...
const Enemy Enemy::WAVE1BASIC(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
...
const Enemy Enemy::DIVER(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
  ...
  if (enemy->cycle(500, 110) || enemy->cycle(500, 210) ||
    ...
    enemy->fireBullets(
        BulletInfo(Bullet::BASIC10, Texture::ELLIPSEORANGE, SpawnInfo(), 2)
						<< BulletInfo(Bullet::BASIC6, Texture::ELLIPSEORANGE,
  ...
  if (enemy->cycle(100)) {
    enemy->fireBullets(
        BulletInfo(Bullet::BASIC7, Texture::ELLIPSERED, SpawnInfo(), 2)
						<< BulletInfo(Bullet::BASIC4, Texture::ELLIPSEORANGE,
													SpawnInfo() << SpawnInfo(-10) << SpawnInfo(10), 2),
        enemy->getDirectionOfEntity(Game::getPlayer()));
  }
});
...
const Enemy Enemy::SUNEASY(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
...
const Enemy Enemy::WHEEL(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
  if (enemy->cycle(1000, 10) || enemy->cycle(1000, 510))
    enemy->moveTo({0, -100}, 100, QUICK);
  {
    if (enemy->cycle(5)) {
...
const Enemy Enemy::SUN(Texture::ENEMYTEMP, 500, [](EntityEnemy* enemy) {
  if (enemy->cycle(1000, 10) || enemy->cycle(1000, 510))
    enemy->moveTo({0, -100}, 100, QUICK);
  {
    if (enemy->cycle(10)) {
